...
namespace __W_LOG_PLAY
{
	...
	void vWriteLog(int iType,const char *pFun, int line, const char *fmt, void *_va_)
	{
		...
		do
		{
			...
			{ //log.m_pBuffer的长度小于格式化输出的大小
				
				iLen2 = iLimit;

				//此处为增大输出缓存，暂时不需要，直接截断吧。
		...
		if (iDecTimes > 60)
		{
			...
			__W_LOG_PLAY::PrintfSystemInfo();//打印内存占用信息
		}
	...
	int GetFormatLen(const char* fmt, va_list argList)
	{
		...
		int len = vfprintf(fp, fmt, argList) 
			...
			+ 64 // 多加64字节（容错处理：linux下发现有些情况vfprintf()返回的长度会比sprintf()返回的长度少）
		#endif 
			;
#endif
		return len;
	...
	int PrintfSystemInfo()
	{
		...
		ul Mb = 1024 * 1024;
		ul iSysMemPre = (ul)(stat.dwMemoryLoad);//内存占用百分比
